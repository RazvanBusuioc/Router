...
int get_packet(packet *m) {
	...
	while (1) {
		...
		res = select(interfaces[ROUTER_NUM_INTERFACES - 1] + 1, &set, NULL,
		NULL, NULL);
		DIE(res == -1, "select");
...
void incremental_updating_checksum(struct iphdr *ip_hdr){
	/*In practice, this function adds 1 to the checksum field*/
	unsigned short sum = ntohs(ip_hdr->check) + 0x100;
	ip_hdr->check = htons(sum + (sum>>16));
}
